...
local scrollView = scrollView.new{ top=430, bottom=0 }


local collections= { "Home" ,"Custom Collection 1","Custom Collection 2","Custom Collection 3","Custom Collection 4" , "Custom Collection 5"}

local function createCollection(id)
print("Collection :".. id)
 local row
 if(id == "Home") then
   row = display.newRoundedRect( 0, 0, 800, 300, 6 )
 else
  row = display.newRoundedRect( 0, 0, 800, 300, 6 )
 end
  row.alpha = 0.4
...
function scene:create(event)
  ...
  local row = createCollection(collections[i])
  rows:insert(row)
  rows.x = display.contentWidth*0.5 -110
  if(id =="Home") then
	rows.y = last + math.floor( 100 + (rows.height + 40)*(i-1) + 40 )
  else
   rows.y =  math.floor( 20+ (rows.height + 24)*(i-1) + 24 )
  end

  dragItem16 = display.newImageRect("appDrawer/collection/app_A1.png",111, 111 ,"red")
    dragItem16:setFillColor( 0.5 )
    drop_item:setSize(dragItem16,200,200)

  	--local t = display.newText( decideBtn[i], 0, 0, "Helvetica", 14 )
  	--t:setTextColor( 255, 255, 255 )

  	dragItem16.x = -300
  	dragItem16.y = math.floor(rows.height/2) - 90
  	rows:insert( dragItem16 )

if(id == "Custom Collection 1") then
 dragItem16 = drop_item.new("App1" ,"appDrawer/collection/app_A1.png","red","custom1")
 drop_item:setLocation(dragItem16,100,200)
 sceneGroup:insert(dragItem16)


	dragItem16.x = 100--10
	dragItem16.y = 200--math.floor(row.height/2) - 90
	rows:insert( dragItem16 )
...
-- drop_item:setLocation(dragItem17,335,1110)
 ...
 sceneGroup:insert(dragItem19)
  ...
  scrollView:insert( rows )
  last = row.y
  end

appDrawer:insert(scrollView)
sceneGroup:insert(appDrawer)
...
--[[
  ...
  applist.y = applist_height*0.5 +100
...
function scene:show(event)
  ...
  if (phase =="will") then
    dragItem = display.newImageRect(sceneGroup,"appDrawer/apps/movable_app.png",141, 141)
    dragItem.x, dragItem.y = 980, V_CENTER
