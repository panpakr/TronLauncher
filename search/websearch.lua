...
local function goToHome()
  ...
  composer.gotoScene("home.home" ,{effect = "fromBottom" ,time = 500})
end

local function goToLocalSearch()
  composer.removeScene("search.search")
  composer.gotoScene("search.search" ,{effect = "crossFade" ,time = 500})
end
...
function scene:create(event)
  ...
  background = display.newImageRect( sceneGroup , "search/web_search.png",W,H)
  background.x = display.contentCenterX
  background.y = display.contentCenterY

  -- create touch area for gestures
  localview= display.newRect( sceneGroup,H_CENTER-265, V_CENTER+330,H*0.28,150)
  localview.alpha = 0.01
...
function scene:show(event)
  ...
  elseif(phase == "did")  then
    backStack:push("search.websearch")
  ...
  tap = Gesture.newTapGesture(   localview, { id="1 tch 1 tap" ,name = "webview"})
